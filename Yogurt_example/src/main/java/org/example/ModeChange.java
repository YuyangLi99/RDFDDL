 ...
 *  (1) s1 != s2
 *  (2) s1, s2 share the same numeric variable constraints (like x>=55, x<=65, P<=10)
...
public class ModeChange extends BaseBuiltin {
    ...
    // -------------------------------------------------------------------
    // Query => gather shape constraints => e.g. [ "x>=40", "x<=45", "P<=10" ]
    // -------------------------------------------------------------------
    ...
    private List<ModeCondRow> gatherModeConditions(String stateUri) {
        ...
        try(QueryExecution qe = QueryExecutionFactory.create(sparql, m)) {
            ...
            while(rs.hasNext()) {
                ...
                String rec       = sol.contains("record")? sol.getResource("record").getURI() : null;
