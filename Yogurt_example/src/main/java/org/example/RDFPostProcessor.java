...
public class RDFPostProcessor {
    ...
    private static final String MODE_CHANGE2 = EX_STATES + "ModeChange2";
    private static final String MODE_CHANGE3 = EX_STATES + "ModeChange3";
    private static final String MODE_CHANGE4 = EX_STATES + "ModeChange4";
    ...
    public static Model processInferenceModel(Model model, String outputPath) {
        System.out.println("Starting RDF data processing");
        ...
        try {
            ...
            processModeChangeRelationship(outputModel, MODE_CHANGE2, "ModeChange2");
            processModeChangeRelationship(outputModel, MODE_CHANGE3, "ModeChange3");
            processModeChangeRelationship(outputModel, MODE_CHANGE4, "ModeChange4");

            // Save the processed model if outputPath is provided
            if (outputPath != null && !outputPath.isEmpty()) {
                System.out.println("\nSaving processed model to: " + outputPath);
                try (FileOutputStream out = new FileOutputStream(outputPath)) {
                    RDFDataMgr.write(out, outputModel, RDFFormat.TRIG_PRETTY);
                }
    ...
    private static boolean createDetailedRelationship(Model model, Resource state1, Resource state2) {
        // Query devices and modes in both states
        String deviceInfoQuery =
                "PREFIX sh: <http://www.w3.org/ns/shacl#>\n" +
                        ...
                        "      sh:node ?deviceShape\n" +
                        "    ] .\n" +
                        "    BIND(\"Homogenizer\" as ?deviceType)\n" +
                        "  }\n" +
                        "  UNION\n" +
                        "  {\n" +
                        ...
                        "      sh:node ?deviceShape\n" +
                        "    ] .\n" +
                        "    BIND(\"Pasteurizer\" as ?deviceType)\n" +
                        "  }\n" +
    ...
    private static void showExamples(Model model) {
        ...
        String[] originalRelations = {MODE_CHANGE, MODE_CHANGE2, MODE_CHANGE3, MODE_CHANGE4};
